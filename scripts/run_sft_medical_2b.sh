...
OUTPUT_ROOT="${OUTPUT_ROOT:-${STORAGE_ROOT}/outputs/medicalgpt/sft}"
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_ROOT}/${RUN_ID}}"
LOG_ROOT="${LOG_ROOT:-${STORAGE_ROOT}/logs/medicalgpt/sft}"
LOG_FILE="${LOG_FILE:-${LOG_ROOT}/${RUN_ID}.log}"
RUN_NAME="${RUN_NAME:-medical-sft-2b-${RUN_ID}}"
...
echo "开始SFT正式训练：${RUN_ID}"
printf '%q ' "${CMD[@]}"; printf '\n'
set +e
"${CMD[@]}" 2>&1 | tee "${LOG_FILE}"
TRAIN_STATUS="${PIPESTATUS[0]}"
set -e
if [[ "${TRAIN_STATUS}" -ne 0 ]]; then echo "SFT训练失败，退出码：${TRAIN_STATUS}；日志：${LOG_FILE}" >&2; exit "${TRAIN_STATUS}"; fi
echo "SFT训练完成；输出：${OUTPUT_DIR}；日志：${LOG_FILE}"
