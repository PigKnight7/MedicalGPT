...
OUTPUT_ROOT="${OUTPUT_ROOT:-${STORAGE_ROOT}/outputs/medicalgpt/grpo_smoke}"
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_ROOT}/${RUN_ID}}"
LOG_ROOT="${LOG_ROOT:-${STORAGE_ROOT}/logs/medicalgpt/grpo_smoke}"
LOG_FILE="${LOG_FILE:-${LOG_ROOT}/${RUN_ID}.log}"
TENSORBOARD_DIR="${TENSORBOARD_DIR:-${LOG_ROOT}/tensorboard/${RUN_ID}}"
RUN_NAME="${RUN_NAME:-medical-cmexam-grpo-smoke-${RUN_ID}}"
...
MAX_STEPS="${MAX_STEPS:-10}"; NUM_GENERATIONS="${NUM_GENERATIONS:-2}"
MAX_PROMPT_LENGTH="${MAX_PROMPT_LENGTH:-}"; MAX_COMPLETION_LENGTH="${MAX_COMPLETION_LENGTH:-32}"; BETA="${BETA:-0}"
LOGGING_STEPS="${LOGGING_STEPS:-1}"; SAVE_STEPS="${SAVE_STEPS:-5}"; SAVE_TOTAL_LIMIT="${SAVE_TOTAL_LIMIT:-2}"
...
for value in "$PER_DEVICE_TRAIN_BATCH_SIZE" "$GRADIENT_ACCUMULATION_STEPS" "$MAX_STEPS" "$NUM_GENERATIONS" "$MAX_COMPLETION_LENGTH" "$MAX_SAMPLES"; do
...
CMD=(
    ...
    --data_seed "$DATA_SEED" --shuffle_before_select "$SHUFFLE_BEFORE_SELECT" --max_samples "$MAX_SAMPLES"
...
if [[ "$DRY_RUN" == 1 ]]; then
    ...
    printf 'MAX_STEPS=%s MAX_SAMPLES=%s BATCH=%s GRAD_ACCUM=%s NUM_GENERATIONS=%s MAX_COMPLETION_LENGTH=%s BETA=%s\n' \
        "$MAX_STEPS" "$MAX_SAMPLES" "$PER_DEVICE_TRAIN_BATCH_SIZE" "$GRADIENT_ACCUMULATION_STEPS" "$NUM_GENERATIONS" "$MAX_COMPLETION_LENGTH" "$BETA"
...
CURRENT_STAGE="TRL GRPO smoke训练"
set +e
"${CMD[@]}" 2>&1 | tee "$LOG_FILE"
TRAIN_STATUS="${PIPESTATUS[0]}"
set -e
if [[ "$TRAIN_STATUS" -ne 0 ]]; then
    echo "GRPO smoke训练失败，退出码=$TRAIN_STATUS；日志=$LOG_FILE；输出=$OUTPUT_DIR" >&2; exit "$TRAIN_STATUS"
fi
printf 'GRPO smoke训练完成。\n输出目录：%s\n日志文件：%s\n起始adapter：%s\n数据文件：%s\n' \
