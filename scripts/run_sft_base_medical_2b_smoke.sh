...
OUTPUT_ROOT="${OUTPUT_ROOT:-${STORAGE_ROOT}/outputs/medicalgpt/sft_base_smoke}"
OUTPUT_DIR="${OUTPUT_DIR:-${OUTPUT_ROOT}/${RUN_ID}}"
LOG_ROOT="${LOG_ROOT:-${STORAGE_ROOT}/logs/medicalgpt/sft_base_smoke}"
LOG_FILE="${LOG_FILE:-${LOG_ROOT}/${RUN_ID}.log}"
RUN_NAME="${RUN_NAME:-medical-sft-base-2b-smoke-${RUN_ID}}"
...
CMD=(
    ...
    # 不提供已有adapter路径，入口会在Base模型上调用get_peft_model新建adapter。
    --use_peft True
    ...
    --logging_strategy steps --logging_steps "${LOGGING_STEPS}" --logging_first_step True
    --report_to "${REPORT_TO}"
...
# DRY_RUN位于所有环境和文件检查之前，不访问模型、数据或GPU。
if [[ "${DRY_RUN:-0}" == 1 ]]; then
...
echo "开始Base→SFT冒烟测试：${RUN_ID}"
printf '%q ' "${CMD[@]}"; printf '\n'
set +e
"${CMD[@]}" 2>&1 | tee "${LOG_FILE}"
TRAIN_STATUS="${PIPESTATUS[0]}"
set -e
if [[ "${TRAIN_STATUS}" -ne 0 ]]; then echo "Base→SFT冒烟测试失败，退出码：${TRAIN_STATUS}；日志：${LOG_FILE}" >&2; exit "${TRAIN_STATUS}"; fi
echo "Base→SFT冒烟测试完成；输出：${OUTPUT_DIR}；日志：${LOG_FILE}"
